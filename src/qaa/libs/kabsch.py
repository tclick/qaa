...
class Kabsch(TransformerMixin, BaseEstimator):
    def __init__(self, verbose: bool = True):
        ...
        """
        self.rotation_: ArrayType
        self.translation_: ArrayType
        self.error: float
        self.verbose = verbose

    def fit(self, mobile: ArrayType, reference: ArrayType) -> Kabsch:
        ...
        """
        mobile: ArrayType = check_array(mobile, copy=self.verbose)
        reference: ArrayType = check_array(reference, copy=self.verbose)
        new_mobile, _ = self._center(mobile)
        new_ref, ref_mean = self._center(reference)

        self.rotation_: ArrayType = self._fit(new_mobile, new_ref)
        self.translation_ = ref_mean.copy()

        return self

    def transform(
        self, mobile: ArrayType, reference: Optional[ArrayType] = None
    ) -> ArrayType:
        ...
        mobile: ArrayType = StandardScaler(with_std=False).fit_transform(mobile)
        new_mobile: ArrayType = mobile @ self.rotation_ + self.translation_
        self.error: float = rmse(mobile, new_mobile)
        return new_mobile

    def _center(self, positions: ArrayType) -> Tuple[ArrayType, ArrayType]:
        scale = StandardScaler(with_std=False)
        new_positions: ArrayType = scale.fit_transform(positions)
        return new_positions, scale.mean_

    def _fit(self, mobile: ArrayType, reference: ArrayType) -> NoReturn:
        ...
        * computation of the optimal rotation matrix `rot_matrix`
        ...
        covar: ArrayType = mobile.T @ reference
